...
// ── Ortak form alanları ───────────────────────────────────────────────────────
function FormAlanlari({ form, setForm, ihtiyaclar, toggleIhtiyac, gaz, setGaz, yarali, setYarali, gpsYukleniyor, konumAl, fileRef, fotografSec, preview, mod }) {
  return (
    <>
      {/* Fotoğraf — sadece online modda */}
      {mod === "online" && (
        <div
...
// ── SMS kutusu (sadece offline modda) ────────────────────────────────────────
function SmsKutusu({ smsKodu, kopya, setKopya }) {
  if (!smsKodu) return null
  return (
    <div className="bg-orange-950 border border-orange-700 rounded-xl p-4 space-y-3">
      <div className="flex items-start gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Kodu kopyalayıp <strong className="text-white">+1 (978) 754-6496</strong> numarasına SMS olarak gönderin.
          </p>
        </div>
      </div>
      ...
      <button
        type="button"
        onClick={() => { navigator.clipboard.writeText(smsKodu); setKopya(true); setTimeout(() => setKopya(false), 2500) }}
      ...
      <a href={`sms:+19787546496&body=${encodeURIComponent(smsKodu)}`}
...
export default function IhbarForm() {
  const [mod, setMod]               = useState("online")
  ...
  const smsKodu = mod === "offline" && form.adres
    ? smsCodOlustur(form, ihtiyaclar, gaz, yarali)
    : ""
  ...
  // ── BAŞARI EKRANI (sadece online mod sonrası) ─────────────────────────────
  if (sonuc) {
    ...
    const seviyeText = oncelik >= 70 ? "text-red-400" : oncelik >= 40 ? "text-orange-400" : "text-green-400"
      ...
      <div className="w-full bg-gray-950 min-h-full">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-4">
          ...
          <div className={`rounded-2xl p-5 border space-y-4 ${seviyeRenk}`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-gray-900/70 rounded-xl p-3 text-center">
                ...
                <p className={`text-sm font-bold ${seviyeText}`}>{seviye}</p>
              </div>
              <div className="bg-gray-900/70 rounded-xl p-3 text-center">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-2 text-sm text-gray-300">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Şu an ne yapmalısınız?</p>
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-5 space-y-4">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">
            Formu doldurun — koordinasyon merkezine iletir.
          </p>
        </div>

        {/* Mod seçimi */}
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-1 flex gap-1">
          <button onClick={() => setMod("online")}
            className={`flex-1 py-2.5 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 transition
              ${mod === "online" ? "bg-red-700 text-white" : "text-gray-400 hover:text-white"}`}>
            🌐 <span>İnternetli</span>
          </button>
          <button onClick={() => setMod("offline")}
            className={`flex-1 py-2.5 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 transition
              ${mod === "offline" ? "bg-orange-700 text-white" : "text-gray-400 hover:text-white"}`}>
            📡 <span>İnternetsiz (SMS)</span>
          </button>
        </div>

        {/* Offline açıklama */}
        {mod === "offline" && (
          <div className="bg-orange-950/50 border border-orange-800 rounded-xl p-3 text-xs text-orange-300 flex items-start gap-2">
            <span className="text-base mt-0.5">📡</span>
            <span>
              Formu doldurun, oluşan SMS kodunu kopyalayın ve
              <strong className="text-white mx-1">+1 (978) 754-6496</strong>
              numarasına gönderin. İnternet gerekmez.
            </span>
          </div>
        )}
        ...
        {mod === "online" ? (
          <form onSubmit={gonder} className="space-y-4">
            <FormAlanlari
              ...
              ihtiyaclar={ihtiyaclar} toggleIhtiyac={toggleIhtiyac}
              gaz={gaz} setGaz={setGaz}
              yarali={yarali} setYarali={setYarali}
              gpsYukleniyor={gpsYukleniyor} konumAl={konumAl}
              fileRef={fileRef} fotografSec={fotografSec} preview={preview}
              mod="online"
            />
          ...
          <div className="space-y-4">
            ...
            />
            {smsKodu
              ? <SmsKutusu smsKodu={smsKodu} kopya={kopya} setKopya={setKopya} />
              : (
                <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 text-center text-gray-500 text-sm">
                  📝 Adres girin — SMS kodu otomatik oluşacak
                </div>
              )
            }
          </div>
